...
Node.prototype.drawNode = function() {
    let color = `rgba(${this.color}, ${this.brightness})`;
    ctx.beginPath();
    ctx.arc(this.x, this.y, 2 * this.radius + 2 * this.siblings.length / SIBLING_LIMIT, 0, circle);
    ctx.fillStyle = color;
    ctx.fill();
}
...
Node.prototype.moveNode = function() {
    this.energy -= 2;
    if(this.energy <= 0) {
        this.energy = Math.random() * 100; // reset when energy dies out
...
if(!ctx) {
    alert("Please use a modern browser, your browser does not support canvas ;-;");
}
        ...
        for(let j = 0; j < NODES_QTY; j++) {
            ...
            if(node1 !== node2) {
                ...
                if(distance < SENSITIVITY) {
                    if(node1.siblings.length < SIBLING_LIMIT) {
                        node1.siblings.push(node2);
                    } else {
                        let node_sibling_distance = 0;
                        let max_distance = 0;
                        let s;
                        for(let k = 0; k < SIBLING_LIMIT; k++) {
